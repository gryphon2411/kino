...
test('the disabled ticket feature requests only the data-service authority by default', () => {
  const original = {
    ...
    WEB_BFF_POST_LOGOUT_REDIRECT_URI: process.env.WEB_BFF_POST_LOGOUT_REDIRECT_URI,
  ...
  try {
    ...
    delete process.env.WEB_BFF_POST_LOGOUT_REDIRECT_URI;
    ...
    assert.equal(config.postLogoutRedirectUri, 'http://local.kino.com/api/auth/logout/callback');
  } finally {
